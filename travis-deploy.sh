...
curl -X PUT -u "$1" --data $deployZip --header "Content-Type: application/zip" --header "Content-Disposition: attachment; filename=deploy.zip" https://bat-dev-manage-courses-api-app.scm.azurewebsites.net/api/zip/site/wwwroot/App_Data/jobs/triggered/ucas-import/
